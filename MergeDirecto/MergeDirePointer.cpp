...
public:
    Producto(string n = "Sin Nombre", float p = 0.0) {
        nombre = n;
        precio = p;
    }
...
private:
    ...
    void merge(int inicio, int medio, int fin) {
        ...
        for (int i = 0; i < n1; i++)
            *(L + i) = *(listaProductos + inicio + i);

        for (int j = 0; j < n2; j++)
            *(R + j) = *(listaProductos + medio + 1 + j);
...
int main() {
    ...
    cout << "\n--- MOSTRANDO REGISTRO ACTUAL ---" << endl;
    miTienda.mostrarInventario();

    miTienda.ordenarPorPrecio();

    cout << "\n--- INVENTARIO ORDENADO (CON PUNTEROS) ---" << endl;
    miTienda.mostrarInventario();
